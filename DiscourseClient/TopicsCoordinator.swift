...
extension TopicsCoordinator: TopicsCoordinatorDelegate {
    func didSelect(topic: Topic) {
         ...
         */
        let topicsDetailViewModel = TopicDetailViewModel(topicID: topic.id, topicDetailDataManager: topicDetailDataManager)
        let topicsDetailViewController = TopicDetailViewController(viewModel: topicsDetailViewModel)
        topicsDetailViewController.title = NSLocalizedString("Topic Details", comment: "")
        topicsDetailViewModel.viewDelegate = topicsDetailViewController
        topicsDetailViewModel.coordinatorDelegate = self
        
        presenter.pushViewController(topicsDetailViewController, animated: true)
    }
